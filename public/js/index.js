...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.addEventListener('click', function (e) {
    ...
    if (servicesMenu && !servicesMenu.contains(e.target) &&
      servicesToggle && !servicesToggle.contains(e.target)) {
      servicesMenu.classList.add("hidden");
      const servicesArrow = document.getElementById("services-arrow");
      if (servicesArrow) {
        servicesArrow.classList.remove("rotate-180");
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  contactForm.addEventListener('submit', async function (e) {
    ...
    try {
      ...
      errorMessage.textContent = error.message || 'Failed to send message. Please try again later.';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const videoBlock = videoContainer ? videoContainer.querySelector('video') : null;
  ...
  function handleScale() {
    ...
    const visible = Math.max(0, Math.min(1, (windowHeight - rect.top) / (windowHeight * 0.8)));
    const maxW = getMaxW();
